...
bool Sky130InterconnectMux2::ConnectMemoryRowToStack(
  ...
  for (size_t i = 0; i < sorted_memories.size(); ++i) {
    ...
    // Because this is specific to the layout of sky130 dfxtps around the
    // transmission gate, we know that any invalid wire wll collide with the
    // VGND rail or with the CLK net.
    //
    // Instead of the more general, aspirational function to find empty space,
    // we just the 5x5 grid of via position around the intended original for
    // conflicts.
    //
    // port_is_left_of_x is used to more quickly arrive at a solution on the
    // happy path.
    auto find_jig_fn = [&](
      ...
      geometry::Rectangle test_shape = via_with_margin_fn(
          {start_x + jig_x, start_y});
      size_t attempts = 0;
      bool try_vertical = false;
      while (clk_net_shapes.Overlaps(test_shape)) {
        if (attempts > 2) {
          ...
          // Give up.
          LOG(INFO) << "Need to try vertical jiggle to find via location";
        ...
        geometry::Line jig_line({start_x, start_y}, {start_x + jig_x, start_y});
        bool incident = false;
        ...
        test_shape = via_with_margin_fn({start_x + jig_x, start_y});
        attempts++;
      }
      ...
      for (int i : {-1, 1}) {
        jig_y = i * met1_pitch;
        test_shape = via_with_margin_fn({start_x + jig_x, start_y + jig_y});
        if (!clk_net_shapes.Overlaps(test_shape)) {
          return {jig_x, jig_y};
        }
      }

      LOG(ERROR) << "Could not heuristically find appropriate place for a via";
      return {jig_x, jig_y};
    };
    ...
    if (jig_x != 0 || jig_y != 0) {
      // Some of these points may be duplicated if jig_x == 0 or jig_y == 0, but
      // geometry::PolyLine handles that for us.
      geometry::Point p0 = target_centre;
      geometry::Point p1 = {vertical_x + jig_x, target_centre.y()};
      geometry::Point p2 = {vertical_x + jig_x, target_centre.y() + jig_y};
      geometry::Point p3 = {vertical_x, target_centre.y()};
      geometry::Point p4 = {vertical_x, n_tab_centre.y()};
      geometry::Point p5 = n_tab_centre;

      layout->MakeVia("mcon.drawing", p0);
      layout->MakeWire({p0, p1, p2},
                       "met1.drawing",
                       "li.drawing",
                       "met2.drawing",
                       false,
      ...
      layout->MakeWire({p4, p5},
                       ...
                       net_QI,
                       true);
      layout->MakeVia("mcon.drawing", p5);
    } else {
