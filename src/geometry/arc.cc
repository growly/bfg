...
std::vector<Point> Arc::IntersectingPoints(const Line &line) const {
  ...
  // If vertical, we substitute x = k into the circule equation:
  //
  //                (k - x_c)^2 + (y - y_c)^2 = r^2 
  ...
  //       x^2 -2x_c*x  + m^2*x^2 + 2m(c - y_c)*x + (c - y_c)^2 = r^2
  // x^2 -2x_c*x  + m^2*x^2 + 2m(c - y_c)*x + (c - y_c)^2 - r^2 = 0
  //    (1 + m^2)x + (2m(c - y_c) - 2x_c)*x + (c - y_c)^2 - r^2 = 0
  //
...
bool Arc::InArcBounds(const Point &point) const {
  double angle_rad = std::atan2(
      static_cast<double>(point.y()), static_cast<double>(point.x()));
