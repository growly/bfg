...
class Arc : public Shape {
 ...
 private:
  FRIEND_TEST(ArcTest, InArcBounds_PointOnStartAngle);
  FRIEND_TEST(ArcTest, InArcBounds_PointOnEndAngle);
  FRIEND_TEST(ArcTest, InArcBounds_PointInsideRange);
  FRIEND_TEST(ArcTest, InArcBounds_PointOutsideRange);
  FRIEND_TEST(ArcTest, InArcBounds_FullCircle);
  FRIEND_TEST(ArcTest, InArcBounds_QuadrantI);
  FRIEND_TEST(ArcTest, InArcBounds_QuadrantII);
  FRIEND_TEST(ArcTest, InArcBounds_QuadrantIII);
  FRIEND_TEST(ArcTest, InArcBounds_QuadrantIV);
  FRIEND_TEST(ArcTest, InArcBounds_SpanningZero);
  FRIEND_TEST(ArcTest, InArcBounds_NarrowArc);
  FRIEND_TEST(ArcTest, InArcBounds_OffsetCentre);
  FRIEND_TEST(ArcTest, InArcBounds_PointOnAxis);

  // Test whether the angle made to the given point is within the angular
  // bounds of the arc.
  bool InArcBounds(const Point &point) const;
