...
# This script generates address sequences for spice simulations.
...
print('')
print('Random address lookups:')
print('=======================')
# Now we want to generate a random sequence of read addresses.
#
# If we assume that bits are loaded with odd/even parity, then we need to jump
# between odd and even addresses randomly.

K=4
NUM_TEST_ADDRESSES = 100
addresses = []

for i in range(NUM_TEST_ADDRESSES):
  addr = 2 * random.randint(0, 2**(K-1) - 1)
  if i % 2 != 0:
    addr += 1
  addresses.append(addr)

print(addresses)
generators = generate_generators(generate_patterns(addresses))
for generator in generators:
    print(generator)
